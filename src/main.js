...
import { especies, allPeople, casas, orderByName } from './data.js';

// botton1  para ir a la siguiente Pagina
const boton1 = document.getElementById('conocemas');
...
const infoGeneral = allPeople(POTTER);
const houses = casas(POTTER);

console.log(orderByName(POTTER).map((el) => el.name));
...
const manipulation = document.getElementById('sortBy');

manipulation.addEventListener('click', () => {
 document.getElementById('pagina1').style.display = 'none';
 document.getElementById('pagina2').style.display = 'block';
});